...
-- ---- repara `system_version` criada fora da forma da 0089 (migration 0161) ----
--
-- Achado em produção: a tabela existia com PK composta (key text, value text) —
-- sem `id`, `current_sha`, `changelog_raw`, `agent_last_seen_at`,
-- `update_requested_at`, `update_requested_by` — e `system_update_runs` nem
-- existia. Alguém criou `system_version` por fora do baseline, provavelmente
-- reagindo às mensagens de erro do PostgREST uma coluna de cada vez, sem olhar
-- a 0089. `create table if not exists` nunca recria uma tabela que já existe,
-- então reaplicar o baseline só entregava as colunas cobertas por
-- `alter ... add column if not exists` (0093, 0094) — as demais, inclusive
-- `id`, a chave que todo `.eq("id", 1)` do código assume, ficavam faltando
-- pra sempre.
--
-- Tabela de INSTÂNCIA (sem organization_id, ver 0089): não há tenant para
-- perder dado. O formato errado nunca guardou o singleton (id=1) — 0 linhas —,
-- então dropar (só quando falta a coluna `id`) e deixar a criação de baixo
-- reconstruir é seguro; uma 0089 aplicada certinho não é tocada.
do $$
begin
